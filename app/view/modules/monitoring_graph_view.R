box::use(
  shiny[NS, moduleServer, div, renderUI, tags, uiOutput, reactive, tableOutput,
        observeEvent, renderTable, reactiveVal, renderText, textOutput, req],
  shiny.fluent[Text, fluentPage, reactOutput],
  magrittr[`%>%`],
  jsonlite[read_json],
  r2d3[d3Output, renderD3]
)
...
server <- function(id, dropdown) {
  ...
  #### Génération du graph
  output$d3 <- renderUI({
     r2d3::r2d3(data = read_json("app/data/output_m.json"), d3_version = 4,
  ...
  ########### Génération du fichier JSON à Envoyer au Graph
    json <- reactive({
     ...
     writeLines(json_data, "app/data/output_m.json")
   })
   json()
